...
an `N×N` matrix — you pay full quadratic memory to throw most of it away. Sparsity
...
mask** — the claim being that the fast path reproduces `softmax(QKᵀ + mask)V`, not
...
- **Gather-index exactness.** Query blocks select different *numbers* of key blocks
  ...
  to the wrong answer. Hence a separate validity mask.
...
number at one step — what the deliverable literally asks for — would have supported
...
81% of causal-dense — barely sparse, making its tie with dense weakly informative. At
...
the attention sink. Softmax must sum to one, so a head with nothing it wants parks
...
- **Forward pass only**, as 1.5 specifies. The backward pass is implemented and
  ...
  profile differs.
- **`block_size` fixed at 64**, untuned — the obvious first autotuning knob.
